...
def extract_signal(file_list, read_id_dict):
    ...
    if int(args.process) > 1:
        ...
        def _writer(res):
            pid, num = res
            wid = ((pid - 1) % int(args.process)) + 1  # 保证最多 P 个文件
            if not num[0]:
                pbar.update(1)
                return
            try:
                rid, sequence, signal = num.split("\t")
                if "run" in rid:
                    rid = read_id_dict[rid]
                line = "%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
                    rid,
                    base_quality_dict[rid][0],
                    base_quality_dict[rid][1],
                    base_quality_dict[rid][2],
                    base_quality_dict[rid][3],
                    sequence,
                    signal
                )
                with open(f"{args.output}_{wid}", "a") as out:
                    out.write(line)
            except Exception:
                pass
            pbar.update(1)
        ...
        for file in file_list:
            pool.apply_async(get_signal, (file,), callback=_writer)
...
def main():
    ...
    read_id_dict=get_read_id_dict(args.dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dict', required=True, help='Path to sequencing_summary.txt file.')  
